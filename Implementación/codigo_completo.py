...
def next_move(pasillo_a_ir:float,pasillo_act:float, adyacencia:pd.core.frame.DataFrame)->float:
    ...
    for x in range(ady.shape[0]):
        if existe_ruta(pasillo_a_ir,ady.iloc[x,1],adyacencia):
            return ady.iloc[x,1]
...
def ordenar(orden:list, pasillos:np.ndarray):
    ...
    for pasillo in pasillos:
        for x in range(len(orden)):
            if pasillo==objeto_en_pasillo(orden[x],layout):
    ...
    for x in range(ady.shape[0]):
        if ady.iloc[x,1]==p1:
            ...
            if ady.iloc[x,1]==-1:
                ...
                return existe_ruta(p1,ady.iloc[x,1],adyacencia)
            
def ruta(pasillo_a_ir:float,pasillo_act:float)->list:
    x=pasillo_act
    l=[]
    while x!=pasillo_a_ir:
        x=next_move(pasillo_a_ir,x,adyacencia)
        l.append(x)
    return l
...
def tiempo(ordenes:np.ndarray,cant_pickeadores:int, ots:pd.core.frame.DataFrame,
    ...
    while len(ordenes_enum)>=0:
        ...
        if asignar_ordenes(pickeadores,ordenes_enum,cant_pickeadores)==to_end:
            break
        asignar_ordenes(pickeadores,ordenes_enum,cant_pickeadores)
        for x in range(cant_pickeadores):
            ...
            if pickeadores[x]!=0: 
                ...
                if contador[x]==-1:
                    ...
                    next_move=objeto_en_pasillo(ordenes[pickeadores[x]-1][contador[x]],layout)
                if len(ruta(next_move,pasillo_act[x]))==0:#estoy en el pasillo a recoger producto
                    ...
                    if route[0]==-1:
                        ...
                        if ocupado(route[0],pasillo_bool):
